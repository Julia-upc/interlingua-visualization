...
def preprocess_words_data():
    en = WordEmbeddings('en', 'embedding-en15.json')
    en.load_data()
    en.load_umap_embeddings()
    fr = WordEmbeddings('fr', 'embedding-fr15.json')
    fr.load_data()
    fr.load_umap_embeddings()
    es = WordEmbeddings('es', 'embedding-es15.json')
    es.load_data()
    es.load_umap_embeddings()
    de = WordEmbeddings('de', 'embedding-de15.json')
    de.load_data()
    de.load_umap_embeddings()

    logger.info('Preparing output file for EN')
    prepare_output_file_words(en, 'data_words_en15.json')
    logger.info('Preparing output file for FR')
    prepare_output_file_words(fr, 'data_words_fr15.json')
    logger.info('Preparing output file for ES')
    prepare_output_file_words(es, 'data_words_es15.json')
    logger.info('Preparing output file for DE')
    prepare_output_file_words(de, 'data_words_de15.json')
...
if __name__ == '__main__':
    ...
    map_sentences_to_words(matching_sentences,
                           'data_mapping_sentences_words1515.json',
